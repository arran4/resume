...
# Also the same logic for typst-build and manual-gh-release!
text = text.replace('''  typst-build:
    name: Build Typst document
    needs: [route, discover, prepare-release-tag]
    if: ${{ needs.discover.outputs.has_typst == 'true' && (needs.route.outputs.run_build == 'true' || needs.route.outputs.run_release == 'true') }}''', '''  typst-build:
    name: Build Typst document
    needs: [route, discover, prepare-release-tag]
